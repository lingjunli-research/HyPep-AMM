...
for a in target_sequences: #for each sequence in the sequence list
    ...
    if len(ion_file) == 1: #ensures only one file matched
        for b in ion_file:
            ...
            theoretical_list['Mass (1)'] = ion_read['mass']
            theoretical_list['Mass (2)'] = ((theoretical_list['Mass (1)']-H_mass)+(2*H_mass))/2
            theoretical_list['Mass (3)'] = ((theoretical_list['Mass (1)']-H_mass)+(3*H_mass))/3
            theoretical_list['Mass (4)'] = ((theoretical_list['Mass (1)']-H_mass)+(4*H_mass))/4
